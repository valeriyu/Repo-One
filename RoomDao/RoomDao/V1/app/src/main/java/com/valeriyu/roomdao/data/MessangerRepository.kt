...
class MessangerRepository(cont: Context) {
    ...
    private val userPropirtiesDao = Database.instance.userPropirtiesDao()
    val messagesDao = Database.instance.messagesDao()
    ...
    suspend fun saveUserWithPropierties(uwp: UsersWithProperties) {
        Database.instance.withTransaction {
            userDao.insertUsers(listOf(uwp.user))
            if (uwp.properties != null) {
                userPropirtiesDao.insertUserPropirties(listOf(uwp.properties) as List<UsersProperties>)
            }
        }
    }
    ...
    suspend fun getUserWithPropertiesById(id: Long): UsersWithProperties? {
...
// init =================================================================================
    ...
    suspend fun initUsers() {
        ...
        var props: List<UsersProperties> = emptyList()
        userDao.getAllUsers().map {
            props += UsersProperties(
                id = it.id,
                name = "User # ${it.phone.substring(8)}",
                avatar = avatars[(0..3).random()]
            )
        }
        userPropirtiesDao.insertUserPropirties(props)
    }
