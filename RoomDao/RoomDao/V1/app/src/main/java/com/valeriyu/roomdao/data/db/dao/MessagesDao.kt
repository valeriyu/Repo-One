...
/*
@Dao
//abstract class _MessagesDao : BaseDao<List<Messages>> {
abstract class _MessagesDao {
        ...
        SELECT DISTINCT
         ...
         m.*
          ...
          LEFT JOIN users_messages um ON um.message = m.id
            LEFT JOIN users_properties s ON m.sender = s.id
            LEFT JOIN users_properties r ON um.receiver = r.id
            WHERE m.sender = :id OR m.id IN
             (SELECT message FROM users_messages WHERE receiver = :id)
              AND
              m.sender <> um.receiver
            ORDER BY m.sender, created_at
            
                 """
    )
    protected abstract fun getMessgeById(id: Long): LiveData<List<MessagesWithAttachments>>

    fun getDistinctMessageById(id: Long):
            LiveData<List<MessagesWithAttachments>> = getMessgeById(id).getDistinct()
}*/
