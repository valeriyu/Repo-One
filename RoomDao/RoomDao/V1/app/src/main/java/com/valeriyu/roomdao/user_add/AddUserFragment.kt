...
class AddUserFragment : Fragment(R.layout.fragment_user_add) {
    ...
    private fun saveUser() {
        ...
        var uwp = UsersWithProperties(
            user = User(
                id = args.id,
                phone =phone,
                password = password.hashCode()
            ),
            properties = if (name.isEmpty() && avatar.isEmpty()) {
                null
            } else {
                UsersProperties(
                    id = args.id,
    ...
    private fun setExistingUserInfo(uwp: UsersWithProperties?) {
        if (uwp == null) return
        phoneTextField.editText?.setText(uwp.user.phone.orEmpty())
        nameTextField.editText?.setText(uwp.properties?.name.orEmpty())
        avatarTextField.editText?.setText(uwp.properties?.avatar.orEmpty())
    }
