...
abstract class AbstractWarrior(maxHealth:Int,                                       //максимальный уровень здоровья
...
//        get() = _maxHealth
    ...
    override fun attack(war: AbstractWarrior?) {
        ...
        if(__DEBUG ) println("totalDamage = > $totalDamage FireType => $ftName(${amList.count()}), ${amList[0]}")
        if (totalDamage > 0) {
